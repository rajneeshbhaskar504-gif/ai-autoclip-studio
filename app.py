...
# OpenAI Client
OΡΕΝΑΙ_KEY = os.getenv("OPENAI_API_KEY", "YOUR_OPENAI_API_KEY_HERE")
...
def get_viral_timestamps(audio_file_path):
    ...
    # 2. GPT-4o Prompt updated for up to 90 seconds duration
    prompt = f"""
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Find & Cut 90s Viral Short"):
        if OΡΕΝΑΙ_KEY == "YOUR_OPENAI_API_KEY_HERE":
            st.error("Please code ke andar apni OpenAI API Key daalein!")
        else:
